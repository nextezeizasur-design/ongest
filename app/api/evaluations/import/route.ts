...
REGLAS GENERALES:
- Ignorá: nombre del alumno, fecha, puntaje, logos, copyright, pie de página, el ítem de ejemplo (ítem 0).
- Procesá TODAS las secciones y sub-secciones: LISTENING, GRAMMAR, VOCABULARY, READING, WRITING, SPEAKING. También letras A, B, C, D como sub-secciones.
- Incluí TODOS los ítems numerados del 1 en adelante.
- Respondé ÚNICAMENTE con un array JSON válido. Sin texto, sin markdown, sin bloques de código.
- CRÍTICO: usa solo comillas dobles en el JSON. No uses saltos de línea dentro de los valores.

CÓMO IDENTIFICAR LA CONSIGNA:
- La consigna es el texto en negrita antes de los ítems.
- Cada consigna aplica a todos los ítems que la siguen hasta la siguiente consigna.
...
REGLAS POR TIPO:
- MATCH: q_type="multiple_choice", options=opciones columna derecha (a,b,c...), todas is_correct:false
- PUT IN ORDER: q_type="short_answer", body=palabras separadas por /, options:[]
- UNDERLINE/CHOOSE (word1/word2): q_type="multiple_choice", options=[{body:"word1",is_correct:false},{body:"word2",is_correct:false}]
- FIND THE ERROR: q_type="essay", body=oración con error, options:[]
- COMPLETE: q_type="short_answer", options:[]
- REWRITE: q_type="essay", options:[]
...
async function parseWithClaudeVision(
  ...
  if (!res.ok) {
    const errBody = await res.text()
    throw new Error(`Claude Vision error ${res.status}: ${errBody}`)
  }
  ...
  if (parsed.length === 0) {
    ...
    if (match) {
      ...
      if (result && Array.isArray(result)) parsed = result
    }
  }
