'use client'
// app/coordinator/results/[id]/page.tsx
...
// ── Enunciado según tipo — word_order/match como chips ────────────────────
function QuestionBody({ body, type }: { body: string; type: string }) {
  ...
  return <p className="text-sm font-medium text-gray-900 mb-3 leading-relaxed">{body}</p>
}

export default function CoordinatorGradePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      for (const a of sorted) {
        initPoints[a.id] = a.points_earned ?? 0
        initNotes[a.id]  = a.grader_note   ?? ''
  ...
  // Carga URL firmada de audio on-demand
  async function loadAudio(ans: Answer) {
    ...
    if (data?.signedUrl) setAudioUrls(prev => ({ ...prev, [ans.id]: data.signedUrl }))
  ...
  async function handleSave(finalize: boolean) {
    ...
    try {
      // 1. Guardar short_answer / essay / opción múltiple / verdadero-falso
      ...
      for (const ans of answers) {
        ...
        const { error } = await sb.from('answers').update({
      ...
      for (const ans of answers) {
        ...
        if (error) throw new Error(`Error al guardar speaking: ${error.message}`)
      ...
      if (finalize && savedStatus && savedStatus !== 'graded') {
        throw new Error(`El estado no se actualizó (quedó: ${savedStatus}). Verificá permisos RLS.`)
      }
  ...
  if (done) return (
    <div className="flex h-screen flex-col items-center justify-center bg-gray-50 gap-4">
      ...
      <p className="text-sm text-gray-400">Volviendo a resultados…</p>
    </div>
  )
  ...
  const earnedPts    = Object.entries(points).reduce((acc, [id, pts]) => {
    ...
    return acc + Math.min(pts, ans?.questions?.points ?? 0)
  }, 0)
    ...
    <div className="flex h-screen flex-col bg-gray-50">
      ...
      <header className="flex h-14 flex-shrink-0 items-center justify-between border-b border-gray-200 bg-white px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => handleSave(false)} disabled={saving} className="btn-outline text-xs py-1.5">
            Guardar borrador
          </button>
          <button onClick={() => handleSave(true)} disabled={saving} className="btn-brand text-xs py-1.5">
      ...
      {saveError && (
        <div className="bg-red-50 border-b border-red-200 px-6 py-3 flex items-start justify-between gap-4">
          ...
          <button onClick={() => setSaveError(null)} className="text-red-400 hover:text-red-600 text-xs flex-shrink-0">
            Cerrar ✕
          </button>
        </div>
      )}
                    ...
                    {q.options.map(opt => {
                      ...
                      return (
                        <div key={opt.id} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm ${color}`}>
                          <span className={`text-xs font-medium ${opt.is_correct ? 'text-green-700' : isSelected ? 'text-red-700' : 'text-gray-400'}`}>
                            {opt.is_correct ? '✓' : isSelected ? '✕' : '·'}
                          </span>
                          <span className={opt.is_correct ? 'text-green-800' : isSelected ? 'text-red-800' : 'text-gray-600'}>
                            {opt.body}
                          </span>
                          {isSelected && !opt.is_correct && <span className="ml-auto text-xs text-red-600">Respuesta del alumno</span>}
                          {opt.is_correct && <span className="ml-auto text-xs text-green-700">Correcta</span>}
                        </div>
                      )
                    })}
                  </div>
                      ...
                      audioUrls[ans.id] ? (
                        <div>
                          ...
                          <audio
                            ...
                            src={audioUrls[ans.id]} preload="metadata"
                          />
                        </div>
                      ) : (
                        <button
