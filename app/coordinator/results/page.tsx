// app/coordinator/results/page.tsx
...
export default async function CoordinatorResults() {
  ...
  if (ids.length > 0) {
    // ── Paso 2: intentos filtrados por esos IDs ──────────────────────────
    const { data: attempts } = await sb
      .from('attempts')
      .select(`
        *,
        evaluations ( id, title, pass_score ),
        profiles!attempts_student_id_fkey ( first_name, last_name, email )
      `)
        ...
        {pending.length > 0 ? (
          <div className="card p-0 overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-gray-100">
              <h2 className="text-sm font-semibold text-gray-900">Pendientes de corrección</h2>
                  ...
                  <tr key={att.id}>
                    <td>
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {`${att.profiles?.first_name?.[0] ?? ''}${att.profiles?.last_name?.[0] ?? ''}`.toUpperCase()}
                        </div>
                        <div>
                    ...
                    <td className="font-medium text-gray-900">{att.evaluations?.title}</td>
                    <td className="text-gray-600 text-xs">{formatDateTime(att.submitted_at)}</td>
                    <td className="text-gray-600">{formatDuration(att.time_taken_sec)}</td>
                    <td>
                      <a
                        href={`/coordinator/results/${att.id}`}
        ...
        <div className="card p-0 overflow-hidden">
          ...
          {completed.length === 0 ? (
            <div className="py-12 text-center text-sm text-gray-400">
              Sin correcciones finalizadas aún.
            </div>
          ) : (
            <table className="table-base">
              <thead>
